...
public class RedisCache {
     ...
     * redis锁操作
     */
    private static final long DEFAULT_EXPIRE_TIME = 5000L; // 锁的默认过期时间，单位毫秒

    // 获取锁
    public boolean acquireLock(String lockKey, String requestId, long expireTime) {
        // 使用 RedisTemplate 的 opsForValue() 方法，设置键值对，并设置过期时间
        Boolean acquired = redisTemplate.opsForValue().setIfAbsent(lockKey, requestId, expireTime, TimeUnit.MILLISECONDS);
    ...
    public void releaseLock(String lockKey, String requestId) {
        // 获取当前锁的持有者
        String currentRequestId = (String) redisTemplate.opsForValue().get(lockKey);
        // 如果当前持有者与请求者相同，则删除锁
